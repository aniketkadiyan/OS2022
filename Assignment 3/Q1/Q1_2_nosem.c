    ...
    while (numeats < 4)
    {
        ...
        else
        {
            ...
            else if (Pstat[id].left == 1 && Pstat[id].right == 1)
            {
                ...
                // dinner completed set thinking state
                Pstat[id].left = Pstat[id].right = DINNER_COMPLETED;

                // releasing forks
                Forks[lf].taken = Forks[rf].taken = 0;
                // releasing bowl
                Pstat[id].bowl = 0;
                numbowls++;
...
int main()
{
    ...
    void *msg;
    for (i = 0; i < n; i++)
        Forks[i].taken = Pstat[i].left = Pstat[i].right = Pstat[i].bowl = 0;
